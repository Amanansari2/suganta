...
class ProjectPostEditProjectController extends GetxController {
  final ProjectPostEditProjectRepo postEditProjectRepo =
      Get.find<ProjectPostEditProjectRepo>();
  ...
  Future<void> setEditProjectData(ProjectPostModel project) async {
    await Future.delayed(const Duration(milliseconds: 100));

    projectLogId = project.projectLogId;
    projectImageId = project.id;
    AppLogger.log("Project Log Id ---->>> $projectLogId");

    alreadyUploadedProjectImageUrls.clear();
    alreadyUploadedProjectImageUrls.addAll(project.projectImages);

    selectedProjectSubTypeCategory.value = project.project;

    projectNameController.text = project.projectName ?? "";
    projectReraController.text = project.reraRegister ?? "";
    projectAreaController.text = project.area ?? "";
    projectZipCodeController.text = project.zipCode.toString() ?? "";
    projectCountryNameController.text = project.country ?? "";
    projectTotalAreaController.text =
        cleanDecimal(double.tryParse(project.totalArea ?? ""));
    projectTotalTowersController.text = project.totalTowers.toString();
    projectTotalFloorsController.text = project.totalFloors.toString();
    projectConferenceRoomController.text =
        project.noOfConferenceRoom.toString() ?? "";
    projectSeatsController.text = project.noOfSeats.toString() ?? "";
    projectBathRoomsController.text = project.noOfBathroom.toString() ?? "";
    projectParkingSpacesController.text = project.parkingSpace.toString() ?? "";
    projectDescriptionController.text = project.projectDescription ?? "";
    projectSuperBuildUpAreaController.text = project.superArea ?? "";
    projectLiftController.text = project.lift ?? "";
    projectTotalUnitController.text = project.totalUnit ?? "";
    projectSizeController.text = project.projectSize ?? "";
    projectRoomConfigurationController.text = project.noOfBhk ?? "";
    projectBedRoomsController.text = project.noOfBedroom.toString() ?? "";
    projectBalconyController.text = project.noOfBalcony.toString() ?? "";

    selectedProjectFacing.value = project.facing ?? "";

// City

    if (projectCityOptions.isEmpty) {
      await loadCityList();
    }
    selectedProjectCity = projectCityOptions.firstWhereOrNull(
        (city) => city.name.toLowerCase() == project.city.toLowerCase());
    citySearchController.text = selectedProjectCity?.name ?? "";

    // Dropdown selections

    editSelectedReraStatus.value = editReraStatusList.firstWhereOrNull(
        (e) => e.id.toString() == project.reraStatus.toString());

    editSelectedPropertyTypeCommercialList.value =
        editPropertyTypeCommercialList.firstWhereOrNull(
            (e) => e.id.toString() == project.projectType.toString());

    editSelectedPropertyTypeResidentialList.value =
        editPropertyTypeResidentialList.firstWhereOrNull(
            (e) => e.id.toString() == project.projectType.toString());

    editSelectedProjectStatusList.value =
        editProjectStatusList.firstWhereOrNull(
            (e) => e.id.toString() == project.projectStatus.toString());

    editSelectedPossessionStatusList.value =
        editPossessionStatusList.firstWhereOrNull(
            (e) => e.id.toString() == project.possessionStatus.toString());

    editSelectedDevelopmentStageList.value =
        editDevelopmentStageList.firstWhereOrNull(
            (e) => e.id.toString() == project.developmentStage.toString());

    editSelectedZoningStatusList.value = editZoningStatusList.firstWhereOrNull(
        (e) => e.id.toString() == project.zoningStatus.toString());

    editSelectedPermitStatusList.value = editPermitStatusList.firstWhereOrNull(
        (e) => e.id.toString() == project.permitStatus.toString());

    editSelectedEnvironmentClearanceList.value =
        editEnvironmentClearanceList.firstWhereOrNull((e) =>
            e.id.toString() == project.environmentalClearance.toString());

    List<String> amenityIds = [];

    if (project.amenities != null && project.amenities!.isNotEmpty) {
      try {
        final decoded = jsonDecode(project.amenities!);
        if (decoded is List) {
          amenityIds = decoded.map((e) => e.toString()).toList();
        } else if (decoded is String && decoded.contains(',')) {
          amenityIds = decoded.split(',').map((e) => e.trim()).toList();
        }
      } catch (e) {
        amenityIds =
            project.amenities!.split(',').map((e) => e.trim()).toList();
        AppLogger.log("Fallback to string split: $amenityIds");
      }
    }

    selectedCommercialAmenitiesValues.assignAll(
      amenityIds.where((id) => commercialAmenities.values.contains(id)),
    );

    selectedResidentialAmenitiesValues.assignAll(
      amenityIds.where((id) => residentialAmenities.values.contains(id)),
    );

    //---------------- ⬇️ Step 2:

    projectDeveloperNameController.text = project.developerName;
    projectDeveloperPhoneNumber1Controller.text = project.developerPhNo1;
    projectDeveloperPhoneNumber2Controller.text = project.developerPhNo2 ?? "";
    projectDeveloperEmailAddress1Controller.text = project.developerEmail1;
    projectDeveloperEmailAddress2Controller.text =
        project.developerEmail2 ?? "";
    projectContactPersonNameController.text = project.contactPerson;
    projectContactPersonPhoneNumberController.text =
        project.contactPersonNo ?? "";
    projectContactPersonEmailController.text = project.contactPersonEmail ?? "";

    //---------------- ⬇️ Step 3:

    projectTokenAmountController.text = project.paymentSchedule ?? "";
    projectPropertyTaxController.text = project.propertyTaxAnnual.toString();
    projectMaintenanceFeeController.text = project.maintenanceFees.toString();
    projectAdditionalFeeController.text = project.additionalFees.toString();
    projectPriceRangeController.text = project.priceRange.toString();
    projectOccupancyRateController.text = project.accupancyRate.toString();
    projectAnnualRentalIncomeController.text =
        project.annualRentalIncome.toString();
    projectCurrentValuationController.text =
        project.currentValuation.toString();
  }
  ...
  RxList<ProjectImageModel> alreadyUploadedProjectImageUrls =
      <ProjectImageModel>[].obs;

  RxBool isSelecting = false.obs;
  RxSet<int> selectedProjectImageIds = <int>{}.obs;

  // ///------------------------------------

  RxList<ProjectDropdownItemModel> editReraStatusList =
      <ProjectDropdownItemModel>[].obs;
  RxList<ProjectDropdownItemModel> editPropertyTypeCommercialList =
      <ProjectDropdownItemModel>[].obs;
  RxList<ProjectDropdownItemModel> editProjectStatusList =
      <ProjectDropdownItemModel>[].obs;
  RxList<ProjectDropdownItemModel> editPossessionStatusList =
      <ProjectDropdownItemModel>[].obs;
  RxList<ProjectDropdownItemModel> editDevelopmentStageList =
      <ProjectDropdownItemModel>[].obs;
  RxList<ProjectDropdownItemModel> editZoningStatusList =
      <ProjectDropdownItemModel>[].obs;
  RxList<ProjectDropdownItemModel> editPermitStatusList =
      <ProjectDropdownItemModel>[].obs;
  RxList<ProjectDropdownItemModel> editPropertyTypeResidentialList =
      <ProjectDropdownItemModel>[].obs;
  RxList<ProjectDropdownItemModel> editEnvironmentClearanceList =
      <ProjectDropdownItemModel>[].obs;

  Rx<ProjectDropdownItemModel?> editSelectedReraStatus =
      Rx<ProjectDropdownItemModel?>(null);
  Rx<ProjectDropdownItemModel?> editSelectedPropertyTypeCommercialList =
      Rx<ProjectDropdownItemModel?>(null);
  Rx<ProjectDropdownItemModel?> editSelectedProjectStatusList =
      Rx<ProjectDropdownItemModel?>(null);
  Rx<ProjectDropdownItemModel?> editSelectedPossessionStatusList =
      Rx<ProjectDropdownItemModel?>(null);
  Rx<ProjectDropdownItemModel?> editSelectedDevelopmentStageList =
      Rx<ProjectDropdownItemModel?>(null);
  Rx<ProjectDropdownItemModel?> editSelectedZoningStatusList =
      Rx<ProjectDropdownItemModel?>(null);
  Rx<ProjectDropdownItemModel?> editSelectedPermitStatusList =
      Rx<ProjectDropdownItemModel?>(null);
  Rx<ProjectDropdownItemModel?> editSelectedPropertyTypeResidentialList =
      Rx<ProjectDropdownItemModel?>(null);
  Rx<ProjectDropdownItemModel?> editSelectedEnvironmentClearanceList =
      Rx<ProjectDropdownItemModel?>(null);

  RxBool isEditLoadingDropDown = false.obs;

  Future<void> loadEditProjectDroDowns() async {
    isEditLoadingDropDown.value = true;
    final result = await postEditProjectRepo.fetchEditProjectDropDownList();
    ...
    if (result["success"] == true) {
      ...
      editReraStatusList.assignAll(allItems.where((e) => e.type == 1));
      editPropertyTypeCommercialList
          .assignAll(allItems.where((e) => e.type == 2));
      editProjectStatusList.assignAll(allItems.where((e) => e.type == 3));
      editPossessionStatusList.assignAll(allItems.where((e) => e.type == 4));
      editDevelopmentStageList.assignAll(allItems.where((e) => e.type == 5));
      editZoningStatusList.assignAll(allItems.where((e) => e.type == 6));
      editPermitStatusList.assignAll(allItems.where((e) => e.type == 7));
      editPropertyTypeResidentialList
          .assignAll(allItems.where((e) => e.type == 8));
      editEnvironmentClearanceList
          .assignAll(allItems.where((e) => e.type == 9));
    } else {
      Get.snackbar(
          "Error", result["message"] ?? "Failed to load dropdown data");
    }
    isEditLoadingDropDown.value = false;
  }
...
//-- Super Build-Up Area
  ...
  Map<String, dynamic> buildProjectPayload() {
    ...
    Map<String, dynamic> payload = {
      ...
      "rera_status": editSelectedReraStatus.value?.id.toString(),
      ...
      "project_status": editSelectedProjectStatusList.value?.id.toString(),
      "Possession_status":
          editSelectedPossessionStatusList.value?.id.toString(),
      "development_stage":
          editSelectedDevelopmentStageList.value?.id.toString(),
      "permit_status": editSelectedPermitStatusList.value?.id.toString(),
      "total_towers": projectTotalTowersController.text.trim(),
      "total_floors": projectTotalFloorsController.text.trim(),
      "environmental_clearance":
          editSelectedEnvironmentClearanceList.value?.id.toString(),
    ...
    switch (subtypeKey) {
      case "Commercial":
        payload.addAll({
          "project_type":
              editSelectedPropertyTypeCommercialList.value?.id.toString(),
          "zoning_status": editSelectedZoningStatusList.value?.id.toString(),
      ...
      case "Residential":
        payload.addAll({
          "project_type":
              editSelectedPropertyTypeResidentialList.value?.id.toString(),
  ...
  bool validateStep1Fields() {
    ...
    if (shouldShowField("PROPERTYCOMMERCIAL") &&
        editSelectedPropertyTypeCommercialList.value == null) {
      Get.snackbar("Error", "Please select Property Type");
      return false;
    }

    if (shouldShowField("PROPERTYRESIDENTIAL") &&
        editSelectedPropertyTypeResidentialList.value == null) {
      Get.snackbar("Error", "Please select property Type ");
      return false;
    }
  ...
  Future<void> submitEditDetailStep1() async {
    if (!validateStep1Fields()) return;
    isProjectDetailSubmitted = false;

    isLoading(true);
    try {
      final response = await postEditProjectRepo.submitEditStep1ProjectDetail();
      isLoading(false);
      if (response["success"] == true) {
        isProjectDetailSubmitted = true;
        String msg =
            response["data"]?["message"] ?? "project details submitted";
...
//-- Developer Name
  ...
  Future<void> submitEditProjectDetailsStep2() async {
    ...
    isLoadingStep2(true);
    final response = await postEditProjectRepo.submitEditStep2ProjectDetail(
  ...
  Future<void> submitEditProjectDetailsStep4Images(List<File> images) async {
    ...
    final response = await postEditProjectRepo.submitEditStep4ProjectDetail(
        projectImageId: projectImageId!, imageFiles: images);
...
//-- Current Valuation
  ...
  Future<void> submitEditProjectDetailsStep3() async {
    ...
    isLoadingStep3(true);
    final response = await postEditProjectRepo.submitEditStep3ProjectDetail(
      projectLogId: projectLogId!,
    ...
    if (response["success"] == true) {
      ...
      AppLogger.log("Project Success Message Step 3---->>> $successMessage");
      Get.snackbar("Success", successMessage, backgroundColor: AppColor.green);
    } else {
      isProjectDetailSubmittedStep3 = false;
  ...
  Future<void> deleteSelectedImages() async {
    if (selectedProjectImageIds.isEmpty) {
      Get.snackbar("Error", "No Images Selected for deletion");
      return;
    }

    if (projectImageId == null) {
      Get.snackbar("Error", "Missing Project Image Id");
      return;
    }

    final response = await postEditProjectRepo.deleteMultipleProjectImages(
      projectId: projectImageId!,
      imageIds: selectedProjectImageIds.toList(),
    );
    ...
    if (response["success"] == true) {
      ...
      alreadyUploadedProjectImageUrls
          .removeWhere((img) => selectedProjectImageIds.contains(img.id));
      selectedProjectImageIds.clear();
  ...
  Future<void> initializedEditProject() async {
    await loadCityList();
    await loadEditProjectDroDowns();
    final ProjectPostModel project = Get.arguments;
    await setEditProjectData(project);
  }
