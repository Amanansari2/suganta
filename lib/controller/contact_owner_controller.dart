...
class ContactOwnerController extends GetxController {
  ...
  ContactOwnerController(this.propertyId) {
    AppLogger.log(
        "ContactOwnerController initialized with propertyId: $propertyId");
  }

  final ContactOwnerRepository contactOwnerRepository =
      Get.find<ContactOwnerRepository>();
  ...
  RxBool isContactOwnerLoading = false.obs;
  ...
  Future<void> contactOwner() async {
    ...
    final response = await contactOwnerRepository.contactOwner(
    ...
    AppLogger.log("Contact Owner otp message -->>> $message");
  ...
  Future<void> verifyOtp() async {
    ...
    final response = await contactOwnerRepository.verifyOtp(
    ...
    AppLogger.log("lead status -->>> $leadStatus");
    if (response["success"] == true) {
      if (leadStatus == 1) {
        isVerified(true);
        isOtpStep(false);
