...
import 'package:tytil_realty/views/home/widget/list_view_card.dart';
...
class PropertyViewAll extends StatefulWidget {
  final List<dynamic> propertyViewAll;
  final String title;
  final Future<List<dynamic>> Function()? onLoadMore;
  final String? errorMessage;

  const PropertyViewAll(
      {Key? key,
      required this.propertyViewAll,
      required this.title,
      this.onLoadMore,
      this.errorMessage})
      : super(key: key);
  ...
  @override
  State<PropertyViewAll> createState() => _PropertyViewAllState();
}

class _PropertyViewAllState extends State<PropertyViewAll> {
  ...
  void initState() {
    super.initState();
    _originalList = List.from(widget.propertyViewAll);
  ...
  void _applyFilter(String filter, {bool shouldScrollTop = true}) async {
    ...
    switch (filter) {
      ...
      case 'Yesterday':
        filteredList = filteredList.where((prop) {
      ...
      case 'Last 6 Month':
        ...
        filteredList = filteredList.where((prop) {
          final update = getUpdatedDate(prop);
          AppLogger.log(
              '6M Filter |Property: ${prop.id} | Created: $update | Show? ${update?.isAfter(cutoff)}');
          AppLogger.log(
      ...
      case 'Last 1 Year':
        ...
        filteredList = filteredList.where((prop) {
          final update = getUpdatedDate(prop);
          AppLogger.log(
              '1Y Filter |Property: ${prop.id} | Created: $update | Show? ${update?.isAfter(cutoff)}');
          AppLogger.log(
                        ...
                        child: Stack(children: [
                          Scrollbar(
                            ...
                            child: ListView.builder(
                              ...
                              itemCount:
                                  _localList.length + (_isLoadingMore ? 1 : 0),
                              itemBuilder: (context, index) {
                                if (index < _localList.length) {
                                  final property = _localList[index];

                                  final String imageUrl = property.firstImage !=
                                          null
                                    ...
                                    price: property.feature.rentAmount !=
                                        ...
                                        : PriceFormatUtils.formatIndianAmount(
                                            property.feature.pricePerSquareFt ??
                                                0),
                                    onContact: () => AppLogger.log(
                                        "Contact Owner button clicked"),
                                    onGetPhone: () => AppLogger.log(
                                        "Get Phone Number button clicked"),
                                    onTap: () {
                                      Get.toNamed(
